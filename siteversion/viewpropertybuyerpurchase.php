...
<body>
    ...
    <main id="main" class="main">
        ...
        <section class="section" style="background-color:white;color:black;margin-bottom:30px;text-align:center;">
            <div class="content">
                <br /><br />
                <h3 style="text-transform:uppercase;">property purchase view</h3>
                <?php
        $buyerquery=mysqli_query($con,"select * from buyerbids where buyerincre='$usersincre'");
        $buyeritemsno=mysqli_num_rows($buyerquery);
        if($buyeritemsno>0)
        {
            //proceed with viewing of data
            echo'
            ...
            {
                $propertyincre=$buyerdata['propertyincre'];
                $status=$buyerdata['status'];
                if($status==0)
                {
                    $approvalstatus="<strong style='color:red;'>Site Visit Unapproved<strong>";
                }
                else if($status==1)
                {
                    $approvalstatus="<strong style='color:green;'>Site Visit approved<strong>";
                }
                else if($status==2)
                {
                  $approvalstatus="<strong style='color:purple;'>Purchase Successful<strong>";
                }
                else
                {
                  $approvalstatus="<strong style='color:red;'>Status Unidentified<strong>";
                }
                ...
                <tr>
                    ...
                    <td>'.$propertydata['sitevisit'].'</td>
                    <td>'.$propertydata['estate'].'</td>
                    <td>Kshs.'.number_format($propertydata['price']).'/=</td>
                    <td>'.$approvalstatus.'</td>
                    <td>
                        <form action="buyerviewmoredetails.php" method="post">
                            <input type="hidden" value="'.$propertyincre.'" name="propertyincre" required/>
                            <input type="submit" value="More Information" class="btn btn-primary"/>
                        </form>
                    </td>
                    <td>
                        <form action="buyerdeletebid.php" method="post">
                            <input type="hidden" value="'.$buyerdata['incre'].'" name="buyerincre" required/>
        ...
        {
            echo"Dear $firstname, you have not booked a purchase yet, please click on <a href='../index.php'>PURCHASE PROPERTY</a> in order to book a purchase";
        }
        ?>
                <br /><br />
            </div>
