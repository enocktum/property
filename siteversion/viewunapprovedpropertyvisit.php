...
<body>
  ...
  <main id="main" class="main">
    ...
    if(isset($propertyid))
    {
      //start listing property with unapproved buyers
      $propertyquery=mysqli_query($con,"SELECT * FROM property WHERE incre='$propertyid'");
      $propertydata=mysqli_fetch_array($propertyquery);
      $subcountyincre=$propertydata['subcountyid'];
      $subcountyquery=mysqli_query($con,"select * from sub_county where id='$subcountyincre'");
      $subcountydata=mysqli_fetch_array($subcountyquery);
      $buyersquery=mysqli_query($con,"SELECT * FROM buyerbids WHERE propertyincre='$propertyid'");
      echo"
      ...
      ";
      while($buyerdata=mysqli_fetch_array($buyersquery))
      {
        $buyerincre=$buyerdata['buyerincre'];
        $userquery=mysqli_query($con,"SELECT * FROM system_users WHERE incre='$buyerincre'");
        $userdata=mysqli_fetch_array($userquery);
        $buyername=$userdata['first_name']." ".$userdata['last_name'];
        echo"
        <tr>
          <td>".$buyername."</td>
          <td>".$subcountydata['countyname']."</td>
          <td>".$subcountydata['subcounty']."</td>
          <td>".$propertydata['estate']."</td>
          <td>".$propertydata['price']."</td>
          <td>
            <form action='approvesitevisit.php' method='post'>
                <input type='hidden' value='".$buyerincre."' name='buyerbidincre'/>
