...
<body>
  ...
  <main id="main" class="main">
	...
	<section class="section" style="background-color:white;color:black;margin-bottom:30px;text-align:center;">
		...
		<br/><br/>
        <?php
        $buyerincre=$_POST['buyerincre'];
        if(isset($buyerincre))
        {
            //proceed to view data
            $delete=mysqli_query($con,"delete from buyerbids where incre='$buyerincre'");
            if($delete)
            {
            ...
            {
                echo "<script>alert('Oops!! Could not find the item to delete.\nIt might have been removed by the property buyer')</script>";
                echo "<script>window.location.href='viewpropertybuyerpurchase.php';</script>";
            }
